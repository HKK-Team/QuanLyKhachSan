...
namespace DoAn_LTWD_Quan_Ly_Khach_San
{
    public partial class ThanhToan : Form
    {
        ...
        {
            Connection.Open();
            Mysql = "Select PHONG.MAPHONG,KHACHHANG.HOTEN,(curdate() - khachhang.ngayden), ((curdate() - khachhang.NGAYDEN) *phong.DONGIA) FROM PHONG Inner Join KHACHHANG ON PHONG.MAPHONG = KHACHHANG.MAPHONG Where(KHACHHANG.MAPHONG = N'" + cbMaPhong.Text + @"') and(KHACHHANG.HOTEN = N'" + txtHoTen.Text + @"')";
            ...
            {
                lbphong.Text = Read_Data[0].ToString();
                lbngay.Text = Read_Data[2].ToString();
                lbtongtien.Text = Read_Data[3].ToString() + " VND";
                i++;
            }
            Read_Data.Close();
        }
